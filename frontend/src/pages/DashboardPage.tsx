...
export const DashboardPage: React.FC = () => {
  ...
  const fetchDashboard = async (selectedPeriod: string = 'daily') => {
    try {
      setLoading(true);
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2">
        <div>
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-black text-navy-900 dark:text-white tracking-tight">
        ...
        <div className="grid grid-cols-2 sm:flex sm:items-center gap-2.5 w-full sm:w-auto">
          <button
            ...
            className="px-3 sm:px-4 py-2.5 rounded-xl bg-navy-800 hover:bg-navy-900 text-white font-bold text-xs shadow-soft flex items-center justify-center gap-1.5 transition-all hover:scale-[1.02] cursor-pointer"
          >
            <Droplets className="w-3.5 h-3.5 text-aqua-400 shrink-0" />
            <span className="truncate">Harvesting</span>
          </button>

          <button
            onClick={() => navigate('/simulator')}
            className="px-3 sm:px-4 py-2.5 rounded-xl bg-aqua-500 hover:bg-aqua-600 text-white font-bold text-xs shadow-soft flex items-center justify-center gap-1.5 transition-all hover:scale-[1.02] cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between text-xs text-slate-500 dark:text-slate-400 mt-3 pt-3 border-t border-slate-100 dark:border-surface-darkborder gap-2">
              ...
              <span className="font-semibold text-navy-900 dark:text-slate-300 text-[11px] sm:text-xs">
                Formula: P × A({property?.roof_area_sqm || 120} m²) × Runoff
              </span>
            </div>
          </ChartCard>
        </div>
        ...
        <div className="lg:col-span-4 flex flex-col">
          <div className="bg-white dark:bg-surface-darkcard border border-slate-100 dark:border-surface-darkborder rounded-2xl p-5 sm:p-6 shadow-soft flex-1 flex flex-col justify-between">
            <div>
              <h3 className="text-base sm:text-lg font-bold text-navy-900 dark:text-white tracking-tight">Water Availability</h3>
            ...
            <button
              onClick={() => navigate('/simulator')}
              className="w-full py-2.5 rounded-xl bg-slate-50 dark:bg-surface-dark hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200/80 dark:border-surface-darkborder font-bold text-xs flex items-center justify-center gap-1.5 transition-colors mt-2 cursor-pointer"
            >
        ...
        <div className="lg:col-span-6">
          ...
          >
            <div className="h-60 sm:h-64 w-full pt-2">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.harvest_vs_demand} layout="vertical" margin={{ top: 5, right: 15, left: 10, bottom: 5 }}>
                  ...
                  <YAxis type="category" dataKey="name" stroke="#94a3b8" fontSize={11} width={95} tickLine={false} />
                  <Tooltip
                    contentStyle={{
                      backgroundColor: '#0B3558',
                      borderRadius: '12px',
            ...
            <div className="h-40 sm:h-44 w-full pt-2">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data.groundwater_trend.history} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <YAxis stroke="#94a3b8" fontSize={11} domain={[6, 11]} reversed tickLine={false} />
                  <Tooltip
                    contentStyle={{
                      backgroundColor: '#0B3558',
                      borderRadius: '12px',
